...
- **RAND (Random Spam Mode):** Кнопка "RAND" запускает хаотичный спам-режим, отправляя сотни батчей со случайными параметрами: `max_concurrent`, `stress_mode` и тип семафора меняются для каждого батча. Идеально для стресс-тестирования и визуального шоу на хитмапе воркеров.
