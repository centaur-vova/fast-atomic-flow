...
**Проблема**:
При запуске `app` в `onWorkerStart` вызывается `createStream()` и `createConsumer()`.
Если NATS не отвечает, `basis-company/nats` зависает навечно → Swoole дедлок → конь в коме.

3. Добавить try-catch с показом страницы `/kon-not-dead` (503)
